...
ll solve() {
	if(a[0][0] != a[1][0] || a[0][n - 1] != a[1][n - 1]) return -1;
	...
	FOR(i, 1, n - 1) {
		...
		if(a[0][i - 1] != a[0][i + 1] && !to_swap[0].empty()) {
			//DBG(i, to_swap[0].front(), i - to_swap[0].front() + 1);
			ans += (i - to_swap[0].front()) + 1;
			while(true) {
				if(to_swap[0].empty()) break;

				if(!to_swap[1].empty() && to_swap[0].front() > to_swap[1].front()) {
					break;
				}
				to_swap[0].pop();
			}
			//to_swap[0].pop();
			swap(to_swap[0], to_swap[1]);
			a[0][i] = ((a[0][i] - '0') ^ 1) + '0';
		}
		if(!to_swap[1].empty()) {
			if(to_swap[0].empty()) {
				while(!to_swap[1].empty()) to_swap[1].pop();
			}
			else {
				while(!to_swap[1].empty() && to_swap[0].front() > to_swap[1].front()) {
					to_swap[1].pop();
				}
			}
		}
