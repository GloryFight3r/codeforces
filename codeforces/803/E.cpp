...
int solve() {
	...
	vi intervals;
	for(int i = 0; i < n; i++) {
		if(a[1][i] != -1 && a[0][i] - a[1][i] > s) {
			return 0;
		}
		if(a[1][i] == -1)
			intervals.pb(remaining.end() - lower_bound(remaining.begin(), remaining.end(), a[0][i] - s));
	}
	//DBG("DAS");
	...
	for(int i = 0; i < sz(intervals); i++) {
		answer = mul(answer, intervals[i] - i);
		//DBG(intervals[i]);
	}
