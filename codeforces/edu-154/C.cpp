...
bool solve() {
  ...
  for (int i = 0; i < sz(a); i++) {
    if (a[i] == '+') {
      ...
      if (sz(can_be_sorted) > 0 && can_be_sorted.top() == st.top()) {
        can_be_sorted.pop();
      }
      st.pop();
    }
      ...
      if (st.top() > only_sorted) {
        if(sz(can_be_sorted) == 0)
          can_be_sorted.push(st.top());
      }
      else {
        return false;
      }
