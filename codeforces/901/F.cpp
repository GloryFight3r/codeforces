...
double rec(int N, int K) {
  ...
  if (K == 1) {
    return dp[N][K] = 1.00 / N;
  }
  if (dp[N][K] != -1) return dp[N][K];
  return dp[N][K] = 1.00 / N * ((rec(N - 2, K - 2) * (K - 2)) + (rec(N - 2, K - 1) * ((N - 2) - (K - 2))));
}

double dfs(int v) {
  if (v == n) return prb[v] = 1.0;
  if (prb[v] != -1) return prb[v];
  vector <double> ss;
  for (auto x : graph[v]) {
    dfs(x);
    ss.pb(prb[x]);
  }
  sort(ss.begin(), ss.end(), greater<double>());
  double fin_ans = 0;
  int N = sz(ss);
  for (int i = 0; i < sz(ss); i++) {
    ...
    fin_ans += dp[N][i + 1] * ss[i];
  }
  if (N % 2 == 1) {
    fin_ans /= N;
  }
  return prb[v] = fin_ans;
}
