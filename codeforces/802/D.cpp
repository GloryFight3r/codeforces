...
void precompute(vl& precomp) {
	...
	for(int i = 1; i < n; i++) {
		...
		//DBG(up_to, mx_time);
		while(temp && up_to < n) {
			ll to_take = min(temp, 1LL*a[up_to]);
			a[up_to] -= to_take;
			temp -= to_take;
			sum -= to_take;
		...
		if(a[i]) {
			...
			while(a[i]) {
				...
				ll takes = min(to_give, 1LL*a[i]);

				a[i] -= takes;
				sum -= takes;

				if(i + 1 < n && to_give - takes > 0) {
					a[i + 1] -= to_give - takes;
					sum -= to_give - takes;
				}
				cc++;
		...
		precomp.pb(mx_time + cdiv(sum, i + 1));
