...
int findCol(int x, int pos) {
	for(int i = x; i != 0 && i != n + 1; i += pos) {
		if(a[0][i] == '0' || a[2][i] == '0') return -1;
		if(getCount(i) == 3) return i;
	}
	return -1;
}
...
bool noTouch(int x) {
	FOR(i, 0, 3) {
		if(a[i][x] == '1' && a[i][x - 1] == '1') return false;
	}
	return true;
}
...
void solve() {
	FOR(i, 1, n+1) {
		FOR(j, 0, 3) {
			if(a[j][i] == '1') {
				vv.components++;
				if(a[j][i - 1] == '1') {
					vv.connect(j*n+i, j*n+(i-1));
				}
				if(j > 0 && a[j - 1][i] == '1') {
					vv.connect(j*n+i, (j-1)*n+i);
				}
			}
		}
		ans[i] = vv.components;
	...
	while(q--) {
		...
		if(noTouch(l)) {
			...
			// 0
			// 1 case
			if(a[0][l] == '1' && a[1][l] == '0' && a[2][l] == '1') {
				int left = findCol(l, -1);
				int right = findCol(l, 1);

				//ps(left, right);

				// they are connected on the left
				if(left != -1) {
					// they are not connected on the right
					if(right == -1 || right > r)
						answer++;
				}
				else if(a[0][l - 1] == '1' && a[2][l - 1] == '1') {
					answer++;
				}
			}
			else if(getCount(l) == 3) {
				if(a[0][l - 1] == '1' && a[1][l - 1] == '0' && a[2][l - 1] == '1' && findCol(l - 1, -1) == -1) {
					answer++;
				}
			}
			ps(answer);
