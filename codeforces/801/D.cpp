...
//		sum += chd;
		if(contains_cross[x] == 0) {
			lin++;
		}
		else sum += chd;
		//from_children.pb(rec(x, v));
		contains_cross[v] |= contains_cross[x];
	...
	if(cnt > 1) contains_cross[v] = 1;
	if(cnt == 1) return sum;
	return sum + max(0, lin - 1);
}
