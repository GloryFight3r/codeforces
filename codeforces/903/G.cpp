...
void add_if_need(int l) {
  ss.erase(l);

  if (dff[l] == 26 || (dff[l] - 26 == 26 - dff[l + 1])) {
    //DBG(l, "AA");
    ss.ins(l);
  }
}

bool check_is_pal(int l, int r) {
  if (l + 1 > r) {
    return false;
  }
  if (dff[l] == 26) {
...
void solve() {
  ...
  for (int i = 0; i < k; i++) {
    ...
    if (tt == 1) {
      ...
      int t = *ss.lower_bound(l);
      ps(check_is_pal(t, r)?"NO":"YES");
    }
  }
}
