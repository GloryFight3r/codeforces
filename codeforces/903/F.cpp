...
int dfs(int v, int p, int cur_best) {
  vpi vv;
  for (auto x : graph[v]) {
    if (x == p) continue;
    vv.pb({-(bst[x] + 1), x});
  ...
  int ans = max(cur_best, bst[v]);
  for (auto x : graph[v]) {
    if(x == p) continue;

    if (vv[0].s == x) {
      if (sz(vv) > 1) {
        int new_best = max(cur_best, -vv[1].f) + 1;
        
        new_best = max(new_best, mark[v]==1?1:-3*n);
        ans = min(ans, dfs(x, v, new_best));
      }
      else {
        int new_best = max(mark[v]>0?1:-3*n, cur_best + 1);
      ...
      if (sz(vv) != 1) {
        int new_best = max(cur_best, -vv[0].f) + 1;
        new_best = max(new_best, mark[v]==1?1:-3*n);
        ans = min(ans, dfs(x, v, new_best));
      }
      else {
