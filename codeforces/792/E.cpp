...
int check(int x) {
	...
	vi needed(x);
	int br = x, remaining = 0;

	vi vt;

	for(int i = 0; i < n; i++) {
		if(a[i] < x && needed[a[i]] == 0) {
			needed[a[i]] = 1;
			br--;
		}
		else if(a[i] < x) {
	...
	// first use all the ones that are bigger
	
	int z = k;
	int ind = 0;

	while(z && br && ind < sz(vt)) {
		int take = min(z, min(br, vt[ind]));
	...
	int take = min(br, min(remaining, z));
