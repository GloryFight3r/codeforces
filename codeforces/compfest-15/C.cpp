...
int n;
vpi graph[mxN];
int br_ones[mxN], br_zeros[mxN];
int when_calculated[mxN], answer[mxN];
bool calculated[mxN];

int dfs(int v, int& prev_ones) {
  if (calculated[v]) {
    prev_ones = add(prev_ones, br_ones[v]);
    
    return add(answer[v], mul(add(prev_ones, -when_calculated[v]), br_zeros[v]));
  }
  answer[v] = 0;
  calculated[v] = 1;
  for (auto x : graph[v]) {
    if (x.second == 0) {
      answer[v] = add(answer[v], prev_ones);
      br_zeros[v] = add(br_zeros[v], 1);
    }
    else {
      prev_ones = add(prev_ones, 1);
      br_ones[v] = add(br_ones[v], 1);
    }
    answer[v] = add(answer[v], dfs(x.first, prev_ones));
    br_zeros[v] = add(br_zeros[v], br_zeros[x.first]);
    br_ones[v] = add(br_ones[v], br_ones[x.first]);
  }
  when_calculated[v] = prev_ones;
  return answer[v];
}
...
int main() {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    graph[i].resize(k);
    for (int j = 0; j < k; j++) {
      re(graph[i][j].first, graph[i][j].second);
    }
  }
