...
int n, k;
int fact[mxN], inversed[mxN];
int dp[mxN][mxN];

int comb(int N, int K) {
  if (N < 0 || K < 0 || N < K) return 0;
  return mul(fact[N], mul(inversed[K], inversed[N - K]));
}

void solve() {
  fact[0] = 1; inversed[0] = inv(fact[0]);
  FOR(i, 1, mxN) {
    fact[i] = mul(fact[i - 1], i);
    inversed[i] = inv(fact[i]);
  }
  for (int i = 1; i <= k - n + 1; i++) {
    dp[1][i] = comb(min(k, k - n + 1), i);
  }
  dp[1][0] = 1;
  for (int i = 2; i <= n; i++) {
    for (int j = 0; j <= min(k, k - n + i); j++) {
      for (int z = 0; z <= j; z++) {
        dp[i][j] = add(dp[i][j], mul(dp[i - 1][z], comb(min(k, k - n + i) - z, j - z)));
      }
      if (j != 0) {
        dp[i][j] = mul(dp[i][j], mul(i, n - i + 1));
      }
    } 
  }
  int ans = 0;
