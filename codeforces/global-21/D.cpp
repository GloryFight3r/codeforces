...
vi closest_right(T_compare && compare) {
	stack<int> st;
	vi closest(n);

	for(int i = n - 1; i >= 0; i--) {
		while(!st.empty() && !compare(a[st.top()], a[i])) st.pop();
		if(st.empty()) {
			closest[i] = n;
		}
		else {
...
int solve() {
	vi first_smaller = closest_right(less<int>());
	vi first_bigger = closest_right(greater<int>());
	...
	while(ind != n - 1) {
		...
		if(a[ind] < a[ind + 1]) {
			//rising
			while(first_smaller[tmp] > first_bigger[ind]) ind = first_bigger[ind];
		}
		else {
			while(first_bigger[tmp] > first_smaller[ind]) ind = first_smaller[ind];
		}
		ans++;
	}
	return ans;
